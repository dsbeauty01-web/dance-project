...
(async () => {
  ...
  const gapOk = s.revealToWordMs !== null && s.revealToWordMs !== undefined && s.revealToWordMs <= 500;
  check('(b) first word ≤0.5s of face', !!gapOk, 'gap=' + s.revealToWordMs + 'ms (negative = voice led the fade — perceived simultaneous)');
  // (c) backend + fps
