...
def split_name(text: str):
    m = re.match(r"^\s*([A-Za-z\u0590-\u05FF][\w\u0590-\u05FF .'-]{0,30}?):\s+(.*)$", text.strip(), re.S)
    return (m.group(1).strip(), m.group(2).strip()) if m else ("friend", text.strip())
...
def unwrap_envelope(s: str) -> str:
    ...
    field only — otherwise she reads a brace and a quote out loud. Handles a leading
    'Name — ' prefix, and salvages the text when the JSON is truncated mid-string
    (cap_words can cut it), which strict json.loads would throw away.
    """
    ...
    try:
        ...
        pass
    if inner is None:                       # truncated / malformed envelope
    ...
    if not inner:
        return s
    if prefix:                              # 'Refael —' + 'Refael — Link sent' -> no double name
...
def answer(user_text: str, platform: str = "opentalking") -> str:
    ...
    if c["intent"] == "medical":
        say = MEDICAL_DEFLECTION.format(name=first)
